...
客户当前使用或参考的是阿里云 OpenSearch 类搜索方案，核心场景是商品搜索、搜索推荐、搜索运营分析和多语言检索。现有方案能够覆盖传统关键词检索、分词、过滤、排序、下拉提示、同义词和部分运营分析能力，但在 AI 语义理解、跨语言语义召回、搜索链路简化和智能推荐方面存在明显短板。

本项目目标是评估 MongoDB Atlas 是否能够替代并增强现有 OpenSearch 方案，尤其验证 Atlas Search、Atlas Vector Search、MongoDB 聚合分析和 Atlas Charts 是否能共同支撑客户的搜索、推荐、分析与调试需求。

## 2. 客户核心痛点

- 现有 OpenSearch 系统缺乏 AI 能力，主要依赖关键词分词匹配，无法理解用户真实语义意图。
...
这可以替代 OpenSearch 中粗排和精排的部分配置。由于 MongoDB 文档数据与搜索索引在同一平台内，很多场景无需额外搬运候选集到外部服务二次计算。
...
如果主库继续保留 MySQL 或 PolarDB，可通过 Kafka Connect、Debezium、阿里云 DTS 或自研同步服务，将 Binlog 变更同步到 MongoDB Atlas，继续保持秒级更新。
...
当前项目已经围绕 Insta360 商品搜索场景完成 PoC 任务设计，重点验证以下能力：
...
MongoDB Atlas 具备替代并增强阿里云 OpenSearch 类方案的可行性。它不仅可以覆盖传统搜索中的关键词检索、过滤、排序、自动补全、纠错、同义词和运营干预，还可以通过 Atlas Vector Search 补齐客户最关注的 AI 语义理解能力。
...
综合评估，MongoDB Atlas 不只是 OpenSearch 的替代品，更适合作为下一代智能商品搜索与搜索分析平台的基础设施。
